...
(function(){
  ...
  function render(root, tab, opts={}){
    ...
    else if(tab === "career") root.innerHTML = viewCareer();
  ...
  function viewCareer(){
    ...
    const xp = p.xp || 0;
    const next = (window.FlySimStore?.getProgress && (function(){ try{ return (function(){return null;})(); }catch(e){return null;} })()) || null;
    // usamos a mesma fórmula do data.js (espelhada aqui para barra)
    const xpForNext = Math.round(600 + (level*level)*220);
    const frac = Math.max(0, Math.min(1, xpForNext ? (xp/xpForNext) : 0));
    const rep = s.company?.reputation || 0.5;

    const lic = p.licenses || {};
    const badge = (ok)=> `<span class="badge ${ok?"good":"bad"}" style="margin-left:6px">${ok?"OK":"Bloq."}</span>`;
    const licensesHtml = `
      <div class="row"><div>TurboProp</div><div>${badge(!!lic.turboprop)}</div></div>
      <div class="hr"></div>
      <div class="row"><div>Regional</div><div>${badge(!!lic.regional)}</div></div>
      <div class="hr"></div>
      <div class="row"><div>Narrow</div><div>${badge(!!lic.narrow)} <span class="muted small">Nível 2 + Rep 52%</span></div></div>
      <div class="hr"></div>
      <div class="row"><div>Wide</div><div>${badge(!!lic.wide)} <span class="muted small">Nível 4 + Rep 60%</span></div></div>
      <div class="hr"></div>
      <div class="row"><div>Jumbo</div><div>${badge(!!lic.jumbo)} <span class="muted small">Nível 6 + Rep 68%</span></div></div>
    `;

    const statsHtml = `
      <div class="row"><div class="muted">Voos concluídos</div><div><b>${Number(p.totalFlights||0)}</b></div></div>
      <div class="hr"></div>
      <div class="row"><div class="muted">Voos lucrativos</div><div><b>${Number(p.profitableFlights||0)}</b></div></div>
      <div class="hr"></div>
      <div class="row"><div class="muted">Distância total</div><div><b>${Number(p.totalDistanceKm||0).toLocaleString("pt-BR")} km</b></div></div>
      <div class="hr"></div>
      <div class="row"><div class="muted">Rotas abertas</div><div><b>${Number(p.routesOpened||0)}</b></div></div>
      <div class="hr"></div>
      <div class="row"><div class="muted">Equipe</div><div><b>${(s.staff||[]).length}</b></div></div>
      <div class="hr"></div>
      <div class="row"><div class="muted">Melhor lucro diário</div><div><b>${money(p.bestDailyProfit||0)}</b></div></div>
    `;

    return `
      <div class="card">
        <div class="h2">Carreira</div>
        <div class="muted small">Progresso desbloqueia novas classes de aeronaves e rotas. 100% offline.</div>
      </div>
      ...
      <div class="card">
        <div class="row">
          <div>
            ...
            <div class="h2" style="margin:0">Lv ${level}</div>
            <div class="muted small">Reputação: <b>${pct(rep)}</b></div>
          </div>
          <div style="text-align:right">
            <div class="small muted">XP</div>
            <div class="small"><b>${xp.toLocaleString("pt-BR")}</b> / ${xpForNext.toLocaleString("pt-BR")}</div>
          </div>
        </div>
        <div class="hr"></div>
        <div style="height:10px;border-radius:999px;background:rgba(255,255,255,.10);overflow:hidden">
          <div style="height:10px;width:${Math.round(frac*100)}%;background:rgba(255,255,255,.65)"></div>
        </div>
        <div class="muted small" style="margin-top:8px">Ganhe XP concluindo voos (distância + lucro).</div>
      </div>

      <div class="card">
        <div class="h2">Licenças</div>
        <div class="muted small">As licenças liberam compras/rotas por categoria.</div>
        <div class="hr"></div>
        ${licensesHtml}
      </div>

      <div class="card">
        <div class="h2">Estatísticas</div>
        <div class="hr"></div>
        ${statsHtml}
      </div>
      ...
      <div class="card">
        ...
        <div class="muted small">Conclua missões para ganhar caixa e acelerar crescimento.</div>
        <div class="hr"></div>
        <button class="btn primary" data-action="goTab" data-tab="missions">Ir para Missões</button>
      </div>
    `;
  }
  ...
  function viewFleet(){
    ...
    const shop = [...models].sort((a,b)=>a.price-b.price).slice(0, 24).map(m=>{
      const unlocked = window.FlySimStore?.isModelUnlocked?.(m.modelId);
      const req = window.FlySimStore?.unlockReqForCategory?.(m.category) || {minLevel:1,minRep:0.45,license:"regional"};
      const lockLine = unlocked ? `<span class="badge good">Liberado</span>` : `<span class="badge bad">Bloqueado</span>`;
      const reqTxt = unlocked ? "" : `<div class="muted small" style="margin-top:6px">Requer <b>Lv ${req.minLevel}</b> + <b>Rep ${Math.round(req.minRep*100)}%</b> + <b>${escapeHtml(req.license)}</b></div>`;
      return `
        <div class="card">
          <div class="row">
            <div style="display:flex;gap:10px;align-items:center">
            ...
            <div style="text-align:right">
              ...
              <div style="margin-top:6px">${lockLine}</div>
            </div>
          </div>
          <div class="hr"></div>
          <button class="btn primary" data-action="buyAircraft" data-model="${escapeHtml(m.modelId)}" ${unlocked? "":"disabled"}>${unlocked? "Comprar":"Bloqueado"}</button>
        </div>
      `;
    }).join("");
      ...
      ${missions.map(m=>`
        <div class="card">
          <div class="row">
            ...
            <span class="badge ${m.done?"":"good"}">${m.done?"Concluída":"+"+money(m.reward)}</span>
          </div>
        </div>
      `).join("")}
      <div class="card">
  ...
  function checkMissions(){
    ...
    const byCode = new Map((s.airports||[]).map(a=>[a.code,a]));
    const flightsDone = (s.flights||[]).filter(f=>f.status==="FINALIZADO");
    const flightsProfit = flightsDone.filter(f=>f.profit>0);

    function fleetAvgCondition(){
      const fleet = s.fleet||[];
      if(!fleet.length) return 0;
      return fleet.reduce((acc,p)=>acc+(p.condition||0),0)/fleet.length;
    }

    function continentOfCountry(country){
      const c = String(country||"").toUpperCase();
      const AM = new Set(["US","CA","MX","BR","AR","CL","PE","CO","PA","GT"]);
      const EU = new Set(["UK","FR","DE","ES","PT","IT","NL","BE","CH","AT","PL","CZ","GR","SE","NO","FI","DK","IE"]);
      const AS = new Set(["AE","QA","TR","IN","TH","SG","JP","KR","CN","HK","MY","ID","PH"]);
      const AF = new Set(["EG","ZA","KE","ET"]);
      const OC = new Set(["AU","NZ"]);
      if(AM.has(c)) return "AM";
    ...
    for(const m of (s.missions||[])){
      ...
      if(m.type==="fleetSize"){
        ...
        if(flightsProfit.length >= m.target){
          m.done = true; gained += (m.reward||0);
        }
      }else if(m.type==="staffCount"){
        if((s.staff||[]).length >= m.target){
          m.done = true; gained += (m.reward||0);
        }
      }else if(m.type==="repAbove"){
        if((s.company?.reputation||0) >= (m.target||0.7)){
          m.done = true; gained += (m.reward||0);
        }
      }else if(m.type==="cashAbove"){
        if((s.company?.cash||0) >= (m.target||0)){
          m.done = true; gained += (m.reward||0);
        }
      }else if(m.type==="fleetAvgCondition"){
        if(fleetAvgCondition() >= (m.target||0.85)){
          m.done = true; gained += (m.reward||0);
        }
      }else if(m.type==="reachLevelRep"){
        const lv = s.progress?.level || 1;
        const rep = s.company?.reputation || 0.5;
        if(lv >= (m.targetLevel||1) && rep >= (m.targetRep||0.5)){
          m.done = true; gained += (m.reward||0);
        }
      }else if(m.type==="longHaulProfit"){
        const minKm = m.minKm || 6000;
        const count = flightsProfit.filter(f=>(f.distanceKm||0)>=minKm).length;
        if(count >= (m.target||1)){
          m.done = true; gained += (m.reward||0);
        }
      }else if(m.type==="bestDailyProfitAbove"){
        const best = s.progress?.bestDailyProfit || 0;
        if(best >= (m.target||0)){
          m.done = true; gained += (m.reward||0);
        }
      }else if(m.type==="routesInCountry"){
        const country = String(m.country||"BR").toUpperCase();
        const active = (s.routes||[]).filter(r=>r.active).filter(r=>{
          const o = byCode.get(r.origin);
          const d = byCode.get(r.destination);
          return (o?.country||"").toUpperCase()===country && (d?.country||"").toUpperCase()===country;
        }).length;
        if(active >= (m.target||1)){
          m.done = true; gained += (m.reward||0);
        }
      }else if(m.type==="continentsActive"){
        const conts = new Set();
        for(const r of (s.routes||[]).filter(r=>r.active)){
          const o = byCode.get(r.origin);
          const d = byCode.get(r.destination);
          conts.add(continentOfCountry(o?.country));
          conts.add(continentOfCountry(d?.country));
        }
        conts.delete("OT");
        if(conts.size >= (m.target||3)){
          m.done = true; gained += (m.reward||0);
        }
      }
    }

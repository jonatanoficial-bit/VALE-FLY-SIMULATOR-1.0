...
(function(){
  ...
  function clamp01(x) {
    ...
    // ---------- Progressão (Etapa 2) ----------
  const LEVEL_XP_BASE = 600;
  function xpForNext(level){
    const lv = Math.max(1, Number(level||1));
    return Math.round(LEVEL_XP_BASE + (lv*lv)*220);
  }

  function ensureProgress(state){
    state.progress = state.progress || {};
    if(!Number.isFinite(state.progress.level)) state.progress.level = 1;
    if(!Number.isFinite(state.progress.xp)) state.progress.xp = 0;
    state.progress.licenses = state.progress.licenses || {};
    if(typeof state.progress.licenses.turboprop !== "boolean") state.progress.licenses.turboprop = true;
    if(typeof state.progress.licenses.regional !== "boolean") state.progress.licenses.regional = true;
    if(typeof state.progress.licenses.narrow !== "boolean") state.progress.licenses.narrow = false;
    if(typeof state.progress.licenses.wide !== "boolean") state.progress.licenses.wide = false;
    if(typeof state.progress.licenses.jumbo !== "boolean") state.progress.licenses.jumbo = false;

    if(!Number.isFinite(state.progress.totalFlights)) state.progress.totalFlights = 0;
    if(!Number.isFinite(state.progress.profitableFlights)) state.progress.profitableFlights = 0;
    if(!Number.isFinite(state.progress.totalDistanceKm)) state.progress.totalDistanceKm = 0;
    if(!Number.isFinite(state.progress.routesOpened)) state.progress.routesOpened = (state.routes||[]).length;
    if(!Number.isFinite(state.progress.staffHired)) state.progress.staffHired = (state.staff||[]).length;

    if(!Number.isFinite(state.progress.bestDailyProfit)) state.progress.bestDailyProfit = 0;
    if(!Number.isFinite(state.progress.dayProfit)) state.progress.dayProfit = 0;
    if(!Number.isFinite(state.progress.lastProfitDay)) state.progress.lastProfitDay = (state.time?.day||1);

    return state.progress;
  }

  function grantLicenseIfEligible(state){
    const p = state.progress;
    const lv = p.level || 1;
    const rep = state.company?.reputation || 0.5;
    if(lv >= 2 && rep >= 0.52) p.licenses.narrow = true;
    if(lv >= 4 && rep >= 0.60) p.licenses.wide = true;
    if(lv >= 6 && rep >= 0.68) p.licenses.jumbo = true;
  }

  function gainXP(state, amount, reason){
    ensureProgress(state);
    const p = state.progress;
    const add = Math.max(0, Math.round(Number(amount||0)));
    if(add <= 0) return {leveled:false, levelsGained:0};
    ...
    if(levelsGained>0){
      grantLicenseIfEligible(state);
      state.ledger = state.ledger || [];
      state.ledger.unshift({
        ts: Date.now(),
        type: "NIVEL",
        ref: "Carreira",
        detail: `Subiu para nível ${p.level}${reason?(" • "+reason):""}`,
        amount: 0
      });
  ...
  function unlockReqForCategory(cat){
    const c = String(cat||"regional");
    if(c==="turboprop") return {license:"turboprop", minLevel:1, minRep:0.45};
    if(c==="regional")  return {license:"regional",  minLevel:1, minRep:0.45};
    if(c==="narrow")    return {license:"narrow",    minLevel:2, minRep:0.52};
    if(c==="wide")      return {license:"wide",      minLevel:4, minRep:0.60};
    if(c==="jumbo")     return {license:"jumbo",     minLevel:6, minRep:0.68};
    return {license:"regional", minLevel:1, minRep:0.45};
  }

  function isModelUnlocked(state, model){
    ensureProgress(state);
    if(!model) return false;
    const req = unlockReqForCategory(model.category);
    const lv = state.progress.level || 1;
    const rep = state.company?.reputation || 0.5;
    const hasLic = !!state.progress.licenses?.[req.license];
    return hasLic && lv >= req.minLevel && rep >= req.minRep;
  }
...
function computeDemand(route, companyRep) {
  ...
  function buyAircraft(state, modelId) {
    ...
    if (!isModelUnlocked(state, model)) {
      const req = unlockReqForCategory(model.category);
      return {ok:false, reason:`Modelo bloqueado • Requer nível ${req.minLevel} + reputação ${Math.round(req.minRep*100)}% + licença ${req.license}`};
    }
  ...
  // ---------- Conteúdo (Etapa 2) ----------
  const EXTRA_AIRPORTS_STAGE2 = [
    {code:"AMS", city:"Amsterdam", lat:52.3105, lon:4.7683, country:"NL"},
    {code:"FCO", city:"Rome", lat:41.8003, lon:12.2389, country:"IT"},
    {code:"ZRH", city:"Zurich", lat:47.4581, lon:8.5555, country:"CH"},
    {code:"MUC", city:"Munich", lat:48.3538, lon:11.7861, country:"DE"},
    {code:"CPH", city:"Copenhagen", lat:55.6180, lon:12.6508, country:"DK"},
    {code:"ARN", city:"Stockholm", lat:59.6519, lon:17.9186, country:"SE"},
    {code:"OSL", city:"Oslo", lat:60.1939, lon:11.1004, country:"NO"},
    {code:"HEL", city:"Helsinki", lat:60.3172, lon:24.9633, country:"FI"},
    {code:"DUB", city:"Dublin", lat:53.4264, lon:-6.2499, country:"IE"},
    {code:"BRU", city:"Brussels", lat:50.9014, lon:4.4844, country:"BE"},
    {code:"LIS", city:"Lisbon", lat:38.7742, lon:-9.1342, country:"PT"},
    {code:"BCN", city:"Barcelona", lat:41.2974, lon:2.0833, country:"ES"},
    {code:"MXP", city:"Milan", lat:45.6301, lon:8.7231, country:"IT"},
    {code:"VIE", city:"Vienna", lat:48.1103, lon:16.5697, country:"AT"},
    {code:"WAW", city:"Warsaw", lat:52.1657, lon:20.9671, country:"PL"},
    {code:"PRG", city:"Prague", lat:50.1008, lon:14.26, country:"CZ"},
    {code:"ATH", city:"Athens", lat:37.9364, lon:23.9445, country:"GR"},
    {code:"SVO", city:"Moscow", lat:55.9726, lon:37.4146, country:"RU"},

    {code:"GRU2", city:"São Paulo (Campo)", lat:-23.2, lon:-46.9, country:"BR"}, // extra fictício p/ missões regionais
    {code:"CNF", city:"Belo Horizonte", lat:-19.6244, lon:-43.9719, country:"BR"},
    {code:"REC", city:"Recife", lat:-8.1265, lon:-34.9236, country:"BR"},
    {code:"FOR", city:"Fortaleza", lat:-3.7763, lon:-38.5326, country:"BR"},
    {code:"CWB", city:"Curitiba", lat:-25.5285, lon:-49.1758, country:"BR"},
    {code:"MAO", city:"Manaus", lat:-3.0386, lon:-60.0497, country:"BR"},

    {code:"SFO", city:"San Francisco", lat:37.6213, lon:-122.3790, country:"US"},
    {code:"SEA", city:"Seattle", lat:47.4502, lon:-122.3088, country:"US"},
    {code:"DEN", city:"Denver", lat:39.8561, lon:-104.6737, country:"US"},
    {code:"ATL", city:"Atlanta", lat:33.6407, lon:-84.4277, country:"US"},
    {code:"BOS", city:"Boston", lat:42.3656, lon:-71.0096, country:"US"},
    {code:"IAD", city:"Washington", lat:38.9531, lon:-77.4565, country:"US"},

    {code:"HKG", city:"Hong Kong", lat:22.3080, lon:113.9185, country:"HK"},
    {code:"PVG", city:"Shanghai", lat:31.1443, lon:121.8083, country:"CN"},
    {code:"PEK", city:"Beijing", lat:40.0799, lon:116.6031, country:"CN"},
    {code:"KUL", city:"Kuala Lumpur", lat:2.7456, lon:101.7090, country:"MY"},
    {code:"CGK", city:"Jakarta", lat:-6.1256, lon:106.6559, country:"ID"},
    {code:"MNL", city:"Manila", lat:14.5086, lon:121.0198, country:"PH"},

    {code:"NRT", city:"Tokyo (NRT)", lat:35.7720, lon:140.3929, country:"JP"},
    {code:"BNE", city:"Brisbane", lat:-27.3842, lon:153.1175, country:"AU"},
    {code:"MEL", city:"Melbourne", lat:-37.6733, lon:144.8430, country:"AU"},

    {code:"GUA", city:"Guatemala City", lat:14.5833, lon:-90.5275, country:"GT"},
    {code:"BOG", city:"Bogotá", lat:4.7016, lon:-74.1469, country:"CO"},
    {code:"PTY", city:"Panama City", lat:9.0714, lon:-79.3835, country:"PA"},

    {code:"NBO", city:"Nairobi", lat:-1.3192, lon:36.9278, country:"KE"},
    {code:"ADD", city:"Addis Ababa", lat:8.9779, lon:38.7993, country:"ET"}
  ];

  const EXTRA_MISSIONS_STAGE2 = [
    { id:"M3", name:"Licença Narrow", desc:"Alcance reputação 60% e suba para nível 2 (destrava narrow-body).", reward:420000, type:"reachLevelRep", targetLevel:2, targetRep:0.60 },
    { id:"M4", name:"Equipe completa", desc:"Contrate 6 funcionários no total.", reward:260000, type:"staffCount", target:6 },
    { id:"M5", name:"Hub doméstico", desc:"Abra 6 rotas ativas no Brasil.", reward:320000, type:"routesInCountry", target:6, country:"BR" },
    { id:"M6", name:"Cross-continental", desc:"Conclua 3 voos acima de 6.000 km com lucro.", reward:520000, type:"longHaulProfit", target:3, minKm:6000 },
    { id:"M7", name:"Manutenção em dia", desc:"Mantenha a condição média da frota acima de 85%.", reward:280000, type:"fleetAvgCondition", target:0.85 },
    { id:"M8", name:"Reputação Premium", desc:"Chegue a 75% de reputação.", reward:650000, type:"repAbove", target:0.75 },
    { id:"M9", name:"Caixa forte", desc:"Atingir 30 milhões em caixa.", reward:600000, type:"cashAbove", target:30000000 },
    { id:"M10", name:"Licença Wide", desc:"Chegue ao nível 4 e reputação 65% (destrava wide-body).", reward:900000, type:"reachLevelRep", targetLevel:4, targetRep:0.65 },
    { id:"M11", name:"Dia perfeito", desc:"Feche um dia com lucro diário acima de 1.5 milhão.", reward:450000, type:"bestDailyProfitAbove", target:1500000 },
    { id:"M12", name:"Expansão global", desc:"Ative 10 rotas em continentes diferentes (Américas/Europa/Ásia/África/Oceania).", reward:1100000, type:"continentsActive", target:5 }
  ];

  function ensureContentEtapa2(state){
    // aeroportos extras
    const existing = new Set((state.airports||[]).map(a=>a.code));
    for(const a of EXTRA_AIRPORTS_STAGE2){
      if(!existing.has(a.code)){
        (state.airports = state.airports||[]).push(a);
      }
    }

    // missões extras (não duplicar)
    const mset = new Set((state.missions||[]).map(m=>m.id));
    for(const m of EXTRA_MISSIONS_STAGE2){
      if(!mset.has(m.id)){
        (state.missions = state.missions||[]).push(m);
      }
    }
  ...
  function initState() {
    ...
    // migrações/expansões offline (Etapa 2)
    ensureContentEtapa2(state);
  ...
  window.FlySimStore = {
    ...
    isModelUnlocked: (modelId)=> {
      const m = (window.flightData?.aircraftCatalog||[]).find(x=>x.modelId===modelId);
      return isModelUnlocked(window.flightData, m);
    },
    unlockReqForCategory,
    getProgress: ()=> (window.flightData?.progress||{}),
